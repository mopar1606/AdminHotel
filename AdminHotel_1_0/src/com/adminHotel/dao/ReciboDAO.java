...
public class ReciboDAO {
	...
	public Map<String, Object> obtenerDatosParaTicket(Integer idHabitacion) throws Exception {
		...
		String sql = "SELECT r.id_recibo, c.documento, h.numero_habitacion, r.noches, v.total, r.fecha_emision, vd.precio_unitario as precio " +
	            "FROM recibo r " +
	            "JOIN cliente c ON r.id_cliente = c.id_cliente " +
	            "JOIN habitacion h ON r.id_habitacion = h.id_habitacion " +
	            "JOIN venta v ON r.id_venta = v.id_venta " +
	            "JOIN venta_detalle vd ON v.id_venta = vd.id_venta " + 
	            "WHERE r.id_habitacion = ? AND vd.tipo_item = 'HABITACION' " +
	...
	// =============================
    // CARGAR DATOS PARA TICKET (PERSONA ADICIONAL)
    // =============================
    public Map<String, Object> obtenerDatosParaTicketAdicional(Integer idHabitacion) throws Exception {
        ...
        String sql = "SELECT r.id_recibo, c.documento, h.numero_habitacion, r.noches, v.total, r.fecha_emision, vd.precio_unitario as precio " +
                ...
                "JOIN venta v ON r.id_venta = v.id_venta " +
                "JOIN venta_detalle vd ON v.id_venta = vd.id_venta " + 
                "WHERE r.id_habitacion = ? AND vd.tipo_item = 'PERSONA_ADICIONAL' " + // <-- Filtro Clave
                "ORDER BY r.id_recibo DESC LIMIT 1";
       Map<String, Object> datos = null;
	 ...
	 // =============================
	 // REGISTRAR RECIBO DE VENTA MOSTRADOR
	 // Sin habitacion ni cliente especifico
	 // =============================
	 public Integer insertarReciboMostrador(Integer idVenta, Integer idPago) throws Exception {
	     String sql = "INSERT INTO recibo (id_pago, id_venta, fecha_emision) VALUES (?, ?, NOW())";
	     ...
	     throw new Exception("No se pudo registrar el recibo de mostrador.");
	 }
}
